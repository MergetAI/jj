...
emitted immediately. `present()` can be used to suppress these errors.
...
may enable further optimizations in a later pass.

### Resolve referenced commits
...
## Lowering to Resolved Expression

After optimization, the revset is lowered to a `ResolvedExpression` that the
`Index` implementation can evaluate directly. The main differences between a
revset expression and a `ResolvedExpression` are:

* `all()` is converted to `::visible_heads_or_referenced()`.
* Negated expressions like `~x` are converted to
  `::visible_heads_or_referenced() ~ x`.
* Filters outside of intersections are converted to
  `::visible_heads_or_referenced() & filter(f)`.
...
* Filters are represented by a separate `ResolvedPredicateExpression` type which
  ...
  `FilterWithin` operation to handle intersections with filters.

## Evaluation by Index

Finally, the `ResolvedExpression` is passed to the `Index` implementation to be
evaluated. There is a built-in default index implementation, but `jj` can be
extended with alternative implementations.
